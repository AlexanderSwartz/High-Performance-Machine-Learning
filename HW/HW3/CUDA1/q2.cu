...
int main(int argc, char** argv)
{
    ...
    // Host allocation + initialization (timing removed — only kernel timings are kept)
    ...
    // Perform one H->D copy for inputs since inputs don't change between scenarios
    ...
    run_scenario(dim3(1), dim3(1), N, size, &t_warmup_arr[0], &t_kernel_arr[0], 1);
    run_scenario(dim3(1), dim3(256), N, size, &t_warmup_arr[1], &t_kernel_arr[1], 2);
    
    int threadsPerBlock = 256;
    int blocksNeeded = (N + threadsPerBlock - 1) / threadsPerBlock;
    if (blocksNeeded < 1) blocksNeeded = 1;
        run_scenario(dim3((unsigned int)blocksNeeded), dim3((unsigned int)threadsPerBlock), N, size,
