...
__global__ void MatMulKernel(Matrix A, Matrix B, Matrix C){
  ...
  // printf("Thread Block (%d,%d) computes Csub (%d,%d) \n",block_row,block_col,block_row,block_col);
...
#pragma unroll
    for(int e=0; e<BLOCK_SIZE; ++e) {
      ...
      Cvalue2 += shared_A[thread_row][e] * shared_B[e][thread_col + stride];
      Cvalue3 += shared_A[thread_row][e] * shared_B[e][thread_col + 2 * stride];
      Cvalue4 += shared_A[thread_row][e] * shared_B[e][thread_col + 3 * stride];
    }
  ...
  if (thread_col < 4) {
    ...
    Csub[thread_row * C.stride + thread_col + stride] = Cvalue2;
    Csub[thread_row * C.stride + thread_col + 2 * stride] = Cvalue3;
    Csub[thread_row * C.stride + thread_col + 3 * stride] = Cvalue4;
  }
}
