...
int main(int argc, char *argv[])
{
    ...
    int bytesPerIteration = 32; // 8 loads * 4 bytes each
    // Given code assumes N is divisible by 4. Continuing with assumption here...
    int totalIterations = (int)(N / 4);
    double totalBytes = (double)bytesPerIteration * (double)totalIterations * (double)timed;
    double totalGB = totalBytes / 1e9;
    double bandwidth = totalGB / elapsed;

    int flopsPerIteration = 8; // 4 adds + 4 mults
